...
# -*- coding: utf-8 -*-
"""
LLM Analyzer for Gemma 4 26B MoE with FP8 + MTP on A100 40GB

Optimized configuration for:
- Gemma 4 26B MoE model
- FP8 quantization (weights + KV cache)
- Multi-Token Prediction (MTP) for 2-3x speedup
- A100 40GB GPU memory constraints
...
class VLLMOAAS:
    def __init__(self, model_path: str = "/nvmedata/hf_checkpoints/gemma-2-27b-it/",
        ...
        # Initialize Async Engine - Optimized for Gemma 4 26B MoE on A100 40GB with FP8 + MTP
        if speculative_model:
            print(f"Initializing Gemma 4 26B MoE with FP8 + MTP (Multi-Token Prediction)...")
            print(f"Main model: {model_path}")
            print(f"Assistant model: {speculative_model}")
            print(f"Speculative tokens: {num_speculative_tokens}")
        else:
            print(f"Initializing Gemma 4 26B MoE with FP8 quantization...")

        engine_args = AsyncEngineArgs(
            model=model_path,
            trust_remote_code=True,
            tokenizer=model_path,
            # Single GPU configuration (TP=1)
            tensor_parallel_size=1,

            # FP8 Quantization for memory efficiency on 40GB
            dtype="bfloat16",  # Base dtype
            ...
            speculative_model=speculative_model if speculative_model else None,
            num_speculative_tokens=num_speculative_tokens if speculative_model else None,

            # Performance Optimizations
            swap_space=4,  # Reduced swap space
        ...
        if speculative_model:
            print("Model loaded successfully with FP8 quantization + MTP assistant")
        else:
            print("Model loaded successfully with FP8 quantization")
...
def parse_args():
    ...
    parser.add_argument("--output_path", type=str, default="../output/genz_users_interests_gemma4_fp8_mtp.jsonl")
    parser.add_argument("--model_path", type=str, default="/nvmedata/hf_checkpoints/gemma-4-26B-A4B-it")
    parser.add_argument("--batch_size", type=int, default=128)  # Reduced for 40GB
    parser.add_argument("--enable_relay_attention", action="store_true")

    # MTP (Multi-Token Prediction) Arguments
    parser.add_argument("--speculative_model", type=str, default=None,
                        help="Path to MTP assistant model for speculative decoding")
    parser.add_argument("--num_speculative_tokens", type=int, default=5,
                        help="Number of tokens to predict speculatively (3-7 typical)")
